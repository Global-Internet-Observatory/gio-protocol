...
for the exact ID/digest permits deletion. Both statuses assert durable ownership,
not volatile queueing. An exact retry is `ALREADY_STORED`; conflicting bytes under
an existing ID are `REJECTED` without overwrite. `RETRY` remains pending and
`REJECTED` remains preserved for operator-visible handling.
...
the complete ACK set before deletion. SHA-256 is not a replacement for future
